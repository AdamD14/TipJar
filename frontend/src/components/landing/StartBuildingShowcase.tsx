      ...
      <div className="mx-auto max-w-[1480px] px-4 text-[#DDE0DA]">
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
          ...
          <Card title="QR Poster (A4)" hint={profileUrl}>
            <div className="aspect-[210/297] w-full overflow-hidden rounded-[12px] border border-white/10 bg-gradient-to-br from-[#0e3a3b] to-[#0a2223] p-4">
              <div className="flex h-full w-full flex-col rounded-[10px] border border-white/10 p-3">
                ...
                <div className="mt-2 text-center text-[11px] text-[#BCC1B6]">{profileUrl}</div>
              </div>
            </div>
          </Card>
          ...
          <Card title="Live Overlay" hint="test feed + QR">
            <div className="relative aspect-video w-full overflow-hidden rounded-[12px] border border-white/10 bg-[#0A0F0F]">
              <iframe
          ...
          <Card title="Presets" hint="Minimal, Streamer Gold, Clean">
            <div className="grid grid-cols-3 gap-2">
              <PresetTile label="Minimal" className="from-[#0b1b1b] to-[#0f2a2a]" />
              <PresetTile label="Gold" className="from-[#1a1200] to-[#332400] ring-1 ring-[#FFD700]/40" />
              <PresetTile label="Clean" className="from-[#0a1212] to-[#0e1e1e]" />
            </div>
          </Card>

          {/* AI Studio — placeholder */}
          <Card title="AI Studio" hint="A4 copy + layout">
            <div className="flex h-[180px] items-center justify-center rounded-[12px] border border-white/10 bg-gradient-to-br from-[#0c1e1f] to-[#102a2b]">
              <div className="rounded-[12px] border border-white/10 bg-black/30 p-3 text-center text-xs text-[#BCC1B6]">
        ...
        <div className="mt-6 flex items-center gap-3">
          <Link
            href="#start"
            className="rounded-[12px] bg-[#FFD700] px-4 py-2 text-sm font-semibold text-[#003737] transition hover:-translate-y-[1px]"
          >
            Open AI Studio
          </Link>
          <Link
            href="#"
            className="rounded-[12px] border border-[#FFD700] px-4 py-2 text-sm font-medium text-[#FFD700] transition hover:-translate-y-[1px]"
          >
            Read docs
          </Link>
        </div>
  ...
  return (
    <article className="rounded-[16px] border border-white/10 bg-card p-4 md:p-6">
      <div className="mb-3">
        ...
        {props.hint ? <p className="text-[12px] text-[#8F9687]">{props.hint}</p> : null}
      </div>
      {props.children}
    </article>
  );
...
function PresetTile({ label, className }: { label: string; className: string }) {
  return (
    <div
      ...
      aria-label={label}
    >
      <span className="rounded-[8px] border border-white/10 bg-black/30 px-2 py-0.5 text-[10px] text-[#DDE0DA]">
        {label}
      </span>
    </div>
  );
...
function TipWidgetPreview() {
  ...
  return (
    <div className="relative h-[180px] overflow-hidden rounded-[12px] border border-white/10 bg-gradient-to-tr from-[#0f2c2c] to-[#0a1a1a] p-4">
        ...
        <div className="rounded-[12px] border border-white/15 bg-white/5 p-3">
          ...
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="rounded-[10px] bg-[#FFD700] px-3 py-1.5 text-xs font-semibold text-[#003737] hover:-translate-y-[1px] transition"
            >
              Tip now
            </button>
          </div>
        ...
        >
          <div className="w-full max-w-[360px] rounded-[12px] border border-white/10 bg-card p-4">
            ...
            <div className="flex justify-end gap-2">
              ...
              <button
                onClick={() => setOpen(false)}
                className="rounded-[10px] bg-[#FFD700] px-3 py-1.5 text-xs font-semibold text-[#003737]"
              >
                Send tip
              </button>
            </div>
...
function TipModalDemo() {
  const [open, setOpen] = useState(false);
  return (
    <div className="relative h-[180px] overflow-hidden rounded-[12px] border border-white/10 bg-[#081414] p-4">
