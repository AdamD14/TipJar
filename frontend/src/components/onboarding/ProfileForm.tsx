  ...
  return (
    ...
    >
      <label className="block">
        ...
        />
        {errName && <p className="mt-1 text-xs text-red-300">{errName}</p>}
      </label>

      <label className="block">
        <span className="mb-1 block text-sm text-[#DDE0DA]">Language</span>
        <select
          id={idLang}
          value={language}
          onChange={(e) => {
          ...
          className="w-full rounded-xl border border-white/10 bg-white/5 px-3 py-3 text-white focus:outline-none focus:ring-2 focus:ring-[#FFD700]"
        >
      ...
      <label className="block md:col-span-2">
        ...
        <textarea
          ...
          className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-white placeholder:text-white/40 focus:outline-none focus:ring-2 focus:ring-[#FFD700]"
        />
        {errBio && <p className="mt-1 text-xs text-red-300">{errBio}</p>}
        <p id="profile-errors" className="sr-only">
          {errName || errBio}
        </p>
        <p className="mt-1 text-xs text-[#BCC1B6]">
