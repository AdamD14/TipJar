...
export type UserOnboardingState = {
  archetype?: string; // Selected archetype from Step 1 (e.g., "live-streamer")
  industry?: string;
  avatar?: string;
  displayName?: string;
  ...
  persist(
    (set) => ({
      ...
      setArchetype: (archetype) =>
        set((state) => ({ data: { ...state.data, archetype } })),
      setAvatar: (url) =>
        set((state) => ({ data: { ...state.data, avatar: url } })),
      setDisplayName: (name) =>
        set((state) => ({ data: { ...state.data, displayName: name } })),
      reset: () => set(initial),
    }),
    {
