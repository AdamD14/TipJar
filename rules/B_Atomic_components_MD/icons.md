...
Zastosowanie koloru w ikonografii TipJar+ wykracza poza dekorację; pełni funkcję nawigacyjną i informacyjną. System opiera się na trzech filarach: Złocie (Wartość), Fiolecie (Technologia) i Jasnym Turkusie (Struktura).
...
* **Kod**: Token \--gold-400 (\#FFD700) jako główny akcent w interfejsie. Jest to odcień jasny, nasycony, doskonale odcinający się od bazowego tła aplikacji.  
* **Warianty**: Dla subtelniejszych stanów (lub jasnych tłach) wykorzystywane są ciemniejsze tokeny, np. \--gold-900 (\#CC7A06).  
* **Technika**: W ikonie wieloelementowej (np. dłoń wrzucająca monetę), Złoto jest używane jako akcent kluczowy (moneta), podczas gdy reszta (dłoń) może pozostać strukturalna lub również przyjąć kolor złoty w stanie aktywnym (active state).

### **3.2. Fiolet: Język Web3 (Accent)**

Fiolet stał się nieoficjalnym standardem wizualnym dla branży Web3 i NFT. Symbolizuje tajemnicę, kreatywność i technologiczną głębię.

* **Zastosowanie**: Ikony związane z blockchainem, smart kontraktami, NFT, "gas fees" (opłatami sieciowymi) i identyfikacją sieci.  
* **Kod**: Zgodnie z paletą używamy tokenu \--purple-300 (\#4D194D) jako bazowego akcentu pomocniczego. Dla stanów wymagających wyższej jasności (selekcja, hover) wspierany jest tokenem \--purple-100 (\#661B66).  
* **Funkcja Rozróżniająca**: Użycie fioletu pozwala użytkownikowi natychmiast odróżnić, czy dana akcja dotyczy tradycyjnego przelewu bankowego (ikona Złota) czy interakcji z łańcuchem bloków (ikona Fioletowa). Jest to kluczowy element UX zapobiegający błędom poznawczym.

### **3.3. Jasny Turkus: Język Kontekstu**

Bardzo jasne odcienie turkusu stanowią "szkielet" systemu ikonografii obojętnej, odchodząc od czystej, archaicznej bieli.
...
* **Kod**: Jako zamiennik bieli w dark mode używamy \--teal-25 (\#E0F2F2), natomiast dla linii nieaktywnych lub pobocznych obrysów stosujemy \--teal-100 (\#ABE1E1).
...
| C. Nawigacja | 5 | Jasny Turkus | Dom, Kompas, Użytkownik, Siatka |
| D. Akcje | 5 | Jasny Turkus / Złoty | Strzałka, Ołówek, Kosz, Skaner |
| E. Statusy | 4 | Zmienny | Tarcza, Zegar, Wykrzyknik, Oko |
| F. Media/Social | 4 | Jasny Turkus | Dymek, Obraz, Wideo, Glob |
...
* **Kolor**: Moneta w kolorze Złotym, dłoń Jasny Turkus lub Złota (zależnie od stanu).
...
Ikony te są najczęściej używane (pasek dolny, menu boczne), więc muszą być maksymalnie czytelne i neutralne (oparte o jasne turkusy, np. \--teal-25 lub \--teal-100).
...
* **Kolor**: Zawsze Złoty (--gold-400) dla sukcesu finansowego, lub semantycznie \--success-base (\#00E676) w komunikatach systemowych.
...
* **Kolor**: Użycie tokenu \--error-base (\#FF5252) dla błędów lub \--warning-base (\#FF9100) dla ostrzeżeń sieci.

**25\. Ikona: status-pending (Oczekiwanie)**

* **Opis**: Zegar / Czas.  
* **Konstrukcja**: Okrąg. Wskazówki na godzinie 3:00.  
* **Styl**: Okrąg może być przerwany (dashed), aby sugerować obrót (loading). Do tego wsparcie semantycznym tokenem \--info-base (\#66D9E8).
...
* stroke="currentColor": Najważniejszy atrybut. Pozwala na zmianę koloru ikony za pomocą CSS (np. color: var(--gold-400) na rodzicu zmieni ikonę na złotą).  
...
Kolor Fioletowy w postaci tokenu \--purple-300 (\#4D194D) na ciemnoturkusowym tle aplikacji (--teal-800 / \#003737) posiada specyficzny kontrast wizualny. Wymaga on precyzyjnego użycia jako solidny obrys bez cienkich detali. Dla zachowania czytelności ikon (minimum 3:1 dla grafik) w małej skali, system może wymuszać wsparcie fioletu jaśniejszym odcieniem ze skali, np. \--purple-100 (\#661B66), gdy ikona stanowi kluczowy punkt fokusowy. Z kolei Złoto \--gold-400 (\#FFD700) na ciemnym turkusie ma doskonały kontrast i nie wymaga żadnej korekty w trybie dark mode.
...
| Kolor | Złoty (--gold-400) dla finansów, Fiolet (--purple-300 / \--purple-100) dla Web3/Tech. | Nie używaj Złotego dla błędów (użyj \--error-base). Nie używaj Fioletu dla salda w PLN. |
