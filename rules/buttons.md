...
* **Otwarty Dukt (Open Apertures)**: Mukta posiada szerokie otwarcie w literach takich jak „c”, „e” czy „s”. Jest to cecha krytyczna dla czytelności na ekranach o niższej rozdzielczości oraz w trudnych warunkach oświetleniowych, zapobiegając optycznemu zlewaniu się kształtów.  
...
Wymagania specyfikują użycie wagi SemiBold (wartość numeryczna 600\) dla etykiet przycisków. Analiza dostępnych wag rodziny Mukta Malar (ExtraLight, Light, Regular, Medium, SemiBold, Bold, ExtraBold) potwierdza, że wybór ten jest optymalny z punktu widzenia zjawiska irradiacji.
...
Aby przełożyć opis słowny „tło złote, tekst ciemnoturkusowy” na precyzyjny język inżynierii, musimy zdefiniować konkretne wartości przypisane do tokenów, które spełnią wymogi kontrastu.

Kolor Primary Background (Złoto): Wykorzystujemy główny token akcentu \--gold-400 (\#FFD700). Jest to odcień jaskrawy, mocno zwracający uwagę i posiadający wystarczającą luminancję, by pełnić funkcję nadrzędnego tła dla kluczowych wezwań do akcji.

Kolor Primary Text (Ciemny Turkus): Musi on tworzyć maksymalny kontrast ze złotem. Rekomendacja: \--teal-900 (\#001F1F). Jest to kolor o bardzo niskiej luminancji, bliski czerni, ale zachowujący bogatą, morską barwę spójną z marką.
...
* Wniosek: Spełnia to z dużym zapasem wymóg WCAG 2.1 Level AAA. Biorąc pod uwagę, że używamy wagi SemiBold (która jest traktowana łagodniej w normach) oraz relatywnie dużych rozmiarów fontu, jest to zestawienie całkowicie bezpieczne i zgodne z prawem (np. Section 508).

### **3.2. Kolory Secondary: Złoto i Fiolet**

Wymaganie „obramowanie złote/fioletowe” sugeruje istnienie dwóch pod-wariantów przycisku drugorzędnego.

* **Secondary Gold (obramowanie \--gold-400)**: Stosowany na ciemnych tłach (gdzie złoto świeci) lub jako uzupełnienie przycisku Primary na białym tle.  
* **Secondary Purple (obramowanie \--purple-300)**: Fiolet (\#4D194D) jest kolorem stanowiącym bazowy akcent pomocniczy, idealnym do wspierania głównej palety.  
  * Kolor tekstu w przyciskach Secondary: Zazwyczaj dziedziczy kolor obramowania, aby powiązać wizualnie granice z treścią. Zatem tekst będzie odpowiednio złoty (na ciemnym tle) lub fioletowy (--purple-300). Fioletowy tekst na jasnych tłach osiąga świetny wynik kontrastu.

### **3.3. Kolor Destructive (Błąd i Usuwanie)**

Wymóg „odcień czerwony” musi zostać skonkretyzowany z uwzględnieniem psychologii błędu. Czysta czerwień (\#FF0000) jest często zbyt agresywna i powoduje wibrację na ekranie. Zgodnie z systemem, używamy semantycznego tokenu \--error-base (\#FF5252). Dla przycisku Destructive (zazwyczaj outline lub ciemne tło w dark mode):

* Tło: \--error-dark (\#3D1010) \- głębokie tło dla bloków/alertów.  
* Tekst/Border: \--error-base (\#FF5252). Takie połączenie komunikuje destrukcyjność akcji, ale nie krzyczy na użytkownika, zachowując elegancję systemu.

### **3.4. Adaptacja do Trybu Ciemnego (Dark Mode)**

System kolorystyczny musi być elastyczny. W trybie ciemnym (Dark Mode), bazując na globalnym tle \--teal-800, duże powierzchnie czystego złota mogą wymagać ostrożności, aby nie powodować nadmiernego zmęczenia wzroku (eye strain). Zalecana modyfikacja:

* Złoto przy najechaniu i interakcjach może korzystać z odpowiednich wariantów skali (np. \--gold-300 lub \--gold-500), aby zarządzać kontrastem.  
* Kolor „Error” w trybie ciemnym może korzystać z \--error-light (\#FFB4AB) dla samego tekstu, ponieważ jaśniejsza czerwień/róż jest lepiej widoczna na głębokim, ciemnoturkusowym tle.
...
* **Mechanizm**: Wykorzystujemy dedykowane tokeny z palety zamiast ręcznej manipulacji filtrami krycia w CSS.  
* **Dla Złotego Primary**: Przyciemnienie tła za pomocą tokenu \--gold-500 (\#FFC312) daje efekt „gęstości” i solidności. Zwiększa to kontrast tekstu przy najechaniu. Opcjonalnie, do efektu podświetlenia można użyć \--gold-300.  
* **Dla Secondary (Outline)**: Wypełnienie przezroczystego tła kolorem bazowym (złotym lub fioletowym) o bardzo niskim kryciu (np. 10% opacity: rgba(255, 215, 0, 0.1)). Daje to subtelny, elegancki efekt.
...
* **Rozwiązanie (Double Ring)**: Zastosowanie podwójnego obrysu z odstępem (offset).  
  ...
  2. Obrys (Outline): 2-3px solidnej linii w zadeklarowanym kolorze fioletowym (--purple-300 / \#4D194D) lub jasnym turkusowym (--teal-300). To rozwiązanie gwarantuje kontrast niezależnie od tego, czy przycisk znajduje się na białym, czy ciemnym tle.

### **5.4. State: Disabled (Wyłączony)**

Stan ten komunikuje, że akcja jest niemożliwa.

* **Wizualizacja**: Tło zmienia się na neutralną szaro-turkusową bazę (np. \--teal-200 lub wygaszony odpowiednik z palety), a tekst na ciemniejszy turkus (np. \--teal-450).  
* **Uwaga A11y**: Elementy disabled są domyślnie ignorowane przez czytniki ekranowe i klawiaturę. Jednakże, dobra praktyka UX sugeruje czasem pozostawienie elementu fokusowalnego, aby czytnik mógł przeczytać tooltip wyjaśniający, dlaczego przycisk jest nieaktywny.  
* **Stylizacja Secondary Disabled**: Border staje się stonowany z wygaszonego turkusu, tło pozostaje przezroczyste.

### **5.5. State: Loading (Ładowanie) i Problem CLS**

To najbardziej złożony stan techniczny. Wymóg „spinner” musi być zrealizowany bez naruszania struktury strony.
...
* **Spinner Style**: Spinner powinien być minimalistycznym okręgiem SVG z animacją stroke-dasharray, w kolorze tekstu przycisku (np. Ciemnoturkusowy \--teal-900 na Złotym \--gold-400), aby zachować spójność.
...
Wymóg posiadania obramowania Złotego lub Fioletowego wymaga jasnych reguł użycia (Governance), aby uniknąć chaosu.

* **Secondary Gold (obrys \--gold-400)**: Powinien być używany w kontekście „Premium” lub „Brand”. Np. akcja „Zobacz ofertę VIP”, „Dołącz do klubu”. Jest to most pomiędzy głównym CTA a tłem.  
* **Secondary Purple (obrys \--purple-300)**: Powinien być używany dla akcji funkcjonalnych, technicznych lub związanych z procesami. Np. „Edytuj profil”, „Konfiguracja”, „Więcej opcji”. Fiolet w psychologii koloru kojarzony jest z kreatywnością i mądrością, co pasuje do akcji konfiguracyjnych.

### **6.2. Wariant: Icon Button (Z Ikoną)**

Ikony przyspieszają rozpoznawanie funkcji (kognitywne przetwarzanie obrazu jest szybsze niż tekstu).
...
\<button class\="btn btn--primary btn--large" aria-busy\="false"\>  
  \<span class\="btn\_\_content"\>  
    \<svg class\="btn\_\_icon"\>...\</svg\> Zapisz Zmiany  
  \</span\>  
  \<span class\="btn\_\_spinner" aria-hidden\="true"\>  
...
.btn\[aria-busy="true"\].btn\_\_content {  
  opacity: 0; /\* Ukryj tekst, ale zachowaj wymiary\! \*/  
  visibility: hidden;  
}

.btn\[aria-busy="true"\].btn\_\_spinner {  
  opacity: 1;  
  visibility: visible;  
}
...
Przedstawiony raport definiuje kompletny ekosystem przycisku, który wykracza poza zwykłą definicję stylów. Poprzez połączenie złotej estetyki, humanistycznej typografii Mukta Malar i inżynieryjnego podejścia do dostępności i wydajności, uzyskujemy komponent, który jest:

1. **Luksusowy i Spójny**: Dzięki precyzyjnie dobranej palecie \--gold-400 / \--teal-900 i typografii SemiBold.  
2. **Dostępny**: Spełniający normy WCAG AA/AAA, z wyraźnym focusem i odpowiednim kontrastem.  
...
Implementacja tego systemu zapewni aplikacji interfejs klasy premium, który nie tylko wygląda nowocześnie, ale działa w sposób przewidywalny i solidny, budując zaufanie użytkownika przy każdej interakcji. Rekomenduje się, aby niniejsza specyfikacja stanowiła podstawę (Foundation) dla budowy Design Systemu w narzędziach takich jak Figma oraz w bibliotekach komponentów (React/Vue/Angular).