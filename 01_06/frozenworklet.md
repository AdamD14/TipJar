**Ostateczna, podrasowana wersja (żyleta).**

### 1. frozen-grid-worklet.js
  ...
  registerPaint(
    ...
    class {
      ...
      paint(ctx, geom, properties) {
        ...
        // Główna siatka + glitch (jeden path dla wydajności)
        ctx.beginPath();
...
.frozen-grid-background {
  ...
  animation: grid-time 8s linear infinite;
}
...
### 3. useFrozenGridWorklet.ts

```tsx
import { useEffect } from "react";

export const useFrozenGridWorklet = () => {
  useEffect(() => {
    if (!("paintWorklet" in CSS)) return;

    const promise = CSS.paintWorklet.addModule("/frozen-grid-worklet.js");
    promise.catch(err => console.error("Worklet load failed:", err));

    return () => {}; // cleanup niepotrzebny
  }, []);
};
```

### 4. FrozenBackground.tsx
...
export const FrozenBackground: React.FC<FrozenBackgroundProps> = ({
  gridSize = 40,
  lineColor = "#FFD700",
  ...
  return (
    <div
      className="frozen-grid-background"
      style={{
        "--grid-size": gridSize,
...
To jest finalna, najmocniejsza wersja. 