...
describe('TipsService', () => {
  ...
  beforeEach(async () => {
    ...
    config = {
      get: jest.fn((key: string, def?: any) => {
        ...
        if (key === 'CIRCLE_USDC_TOKEN_ID') return 'usdc-token';
        return def;
      }),
    };
  ...
  it('processes USDC tip from fan', async () => {
    usersService.findOneById.mockImplementation((id: string) => {
      if (id === 'creator') return { id, circleWalletId: 'cwC', role: UserRole.CREATOR };
    ...
    circleService.initiateInternalTipTransfer.mockResolvedValue({ circleTransactionId: 'tx', status: 'complete', txHash: '0x1' });

    const result = await service.processNewTip({ amount: '10', creatorId: 'creator', fanId: 'fan' });

    expect(circleService.initiateInternalTipTransfer).toHaveBeenCalled();
    expect(prisma.tip.update).toHaveBeenLastCalledWith(expect.objectContaining({
      where: { id: '1' },
      data: expect.objectContaining({ status: TipStatus.COMPLETED, circleTransferId: 'tx' }),
    }));
  ...
  it('fails when USDC transfer throws', async () => {
    ...
    await expect(service.processNewTip({ amount: '5', creatorId: 'creator', fanId: 'fan' })).rejects.toThrow(InternalServerErrorException);
    expect(prisma.tip.update).toHaveBeenLastCalledWith(expect.objectContaining({
  ...
  it('processes guest fiat tip', async () => {
    usersService.findOneById.mockResolvedValue({ id: 'creator', circleWalletId: 'cwC', role: UserRole.CREATOR });
    prisma.tip.create.mockResolvedValue({ id: '3' });
    prisma.tip.update.mockResolvedValue({ id: '3', status: TipStatus.COMPLETED });

    const result = await service.processNewTip({ amount: '5', creatorId: 'creator', fanId: null, paymentGatewayToken: 'tok' });

    expect(prisma.tip.update).toHaveBeenLastCalledWith(expect.objectContaining({
      where: { id: '3' },
      data: expect.objectContaining({ status: TipStatus.COMPLETED, paymentGatewayChargeId: expect.any(String) }),
    }));
    expect(result.status).toBe(TipStatus.COMPLETED);
  });
  ...
  it('fails guest fiat tip when payment fails', async () => {
    ...
    await expect(service.processNewTip({ amount: '5', creatorId: 'creator', fanId: null, paymentGatewayToken: 'fail' })).rejects.toThrow(InternalServerErrorException);
    expect(prisma.tip.update).toHaveBeenLastCalledWith(expect.objectContaining({
      data: { status: TipStatus.FAILED },
    }));
  });
