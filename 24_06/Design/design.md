...
| :---- | :---- | :---- |
| \--teal-25 | \#E0F2F2 | Najjaśniejsze tła, zamiennik bieli w dark mode |
| \--teal-50 | \#CCF7F4 | Tekst na ciemnym tle, tła w trybie jasnym |
| \--teal-100 | \#ABE1E1 | Subtelne akcenty, ikony nieaktywne, obrysy pomocnicze |
| \--teal-200 | \#76CBCB | Obrysy pól formularzy (default) |
| \--teal-300 | \#3FB5B5 | Elementy graficzne, drugorzędne przyciski |
| \--teal-400 | \#2A8A8A | Stany :hover dla ciemniejszych elementów, fokus |
| \--teal-450 | \#0F7F7F | Pośredni hover/akcent dla elementów interactive |
| \--teal-500 | \#007373 | Interaktywne tła kart, nagłówki sekcji |
| \--teal-600 | \#005959 | :hover dla elementów o wadze 500 |
| \--teal-700 | \#004545 | Tło "Elevated" (karty na kartach), obrysy |
| \--teal-800 | \#003737 | Bazowe tło aplikacji (Dark Mode) |
| \--teal-850 | \#002121 | Tło elementów zagłębionych, cienie |
| \--teal-900 | \#001F1F | Globalne tło \<body\>, najgłębsze cienie |
...
| \--gold-200 | \#FFEA00 | Elementy dekoracyjne, stan :hover dla złotych przycisków (rozjaśnienie) |
...
| :---- | :---- | :---- |
| \--bg-app-global | var(--teal-900) | Tło. Nigdy nie używaj \#000000. |
...
5. ---
   ...
   System Warstw (Z-Index Stacking)  
...
1. System Przycisków (Buttons)  
   ...
   Hover \--gold-300 \--teal-900 none \--shadow-2Active \--gold-500 \--teal-900 none scale(0.98), shadow-1  
   Focus \--gold-400 \--teal-900 2px solid \--purple-300 outline-offset: 2px  
   Disabled var(--teal-850) (Ciemny Turkus) var(--teal-200) none none  
   Loading \--gold-400 Ukryty none Spinner SVG (kolor \--teal-900)  
   Krytyczne WCAG: Nigdy nie używaj białego tekstu na \--gold-400. Kontrast 1.4:1 \= FAIL. Tekst  
   musi być \--teal-900 (kontrast 11.2:1 \= AAA).  
   ...
   Default transparent lub var(--error-dark) var(--error-base)  
   Hover rgba(255, 82, 82, 0.1) var(--error-base)  
   Focus transparent Pierścień var(--error-base)  
...
2. ---
   ...
   System Pól Formularzy (Inputs, Textarea, Select)  
   ...
   Tekst (Default) \--text-primary (var(--teal-25))  
   Placeholder \--text-tertiary (var(--teal-100))  
   ...
   Error \--error-base 0 0 0 4px rgba(255, 82, 82, 0.25) Kolor \--error-base  
   Success \--success-base none Kolor \--success-base  
   Disabled \--teal-700 (przerywane) none Opacity 0.4  
   Walidacja: Tekst błędu pod polem w kolorze \--error-light (\#FFB4AB). Dodatkowo opcjonalna  
   ikona wykrzyknika.  
   2.3 Textarea  
   Właściwość Wartość  
   Padding 16px  
   ...
   2.6 Toggle Switch  
   Właściwość Wartość  
   Tor (Off) var(--teal-850), obramowanie 1px solid \--teal-600  
   Suwak (Off) \--teal-600, pozycja lewo  
   Tor (On) \--purple-300  
   Suwak (On) var(--teal-25), pozycja prawo  
   Animacja \--ease-spring, efekt "rozciągania" suwaka  
3. ---

   System Awatarów (Avatars)  
...
1. System Kart (Cards)  
   ...
   Cień (Hover) \--shadow-2 Dodatkowo 0 0 10px rgba(255, 215, 0, 0.1) (złota poświata)  
...
2. ---
   ...
   System Modali i Szuflad (Modal & Bottom Sheet)  
   ...
   Nagłówek Mukta Malar Bold, 24px, var(--teal-25) lub \--gold-400  
...
3. ---
   ...
   System Dymków i Popoverów (Tooltip & Popover)  
   ...
   Tło rgba(0, 55, 55, 0.9) (var(--teal-800) z 90% opacity)  
   ...
   Właściwość Wartość  
   Tło var(--teal-800)  
   Tekst \--text-primary (var(--teal-25))  
   Padding 16px  
   Border Radius 12px  
   ...
   Sukces Szmaragd / Mięta var(--success-base)  
   Błąd Koral / Jasna Malina var(--error-base)  
   Informacja Cyjan / Błękit var(--info-base)  
   Ostrzeżenie Bursztyn var(--warning-base)  
   ⚠️ Krytyczne dla dostępności: role="alert" tylko dla błędów i ostrzeżeń. Dla Sukcesu i Info  
   używaj role="status".  
5. ---

   Stany Ładowania (Spinner & Skeleton)  
   ...
   Właściwość Wartość  
   Kolor Gradient linear-gradient(135deg, var(--gold-400) 0%, var(--purple-300) 100%)  
   Animacja rotate (liniowa) \+ dash (ease-in-out), czas 1.5s \- 2s  
   5.2 Skeleton Screen  
   Właściwość Wartość  
   Tło Bazowe var(--teal-800) (\#003737)  
   Kolor "Shimmer" var(--teal-700) (\#004545)  
...
1. Publiczny Profil Twórcy (Creator Profile View)  
   Zgodnie z dokumentem tech\_Profil\_twórcy.pdf (str. 1-13).1.1 Layout Desktop (≥1024px)  
   ...
   Zapobieganie Okluzji padding-bottom: calc(72px \+ env(safe-area-inset-bottom)) na  
   ...
   Podświetlenie Nowego \--success-base przez 2s, potem powrót do \--bg-surface-base  
2. ---

   Panel Fana (Fan Dashboard)Zgodnie z dokumentem tech\_fan\_profil.pdf (str. 1-13).  
...
3. ---
   ...
   Katalog Twórców (Explore / Discovery View)  
   Zgodnie z dokumentem tech\_katalog\_3part.pdf (str. 18-22).3.1 Layout Desktop (≥1024px)  
   Właściwość Wartość  
...
**⏭️ 🐋 🧬 Krok 5: Specyfikacja Logiki Web3 i Płatności1. Architektura Modala Płatności (Payment Modal) Zgodnie z dokumentem tech\_modal.pdf (str. 1-13) oraz Master Plan UI (str. 1-12). 1.1 Struktura Krok po Kroku (Wizard) Krok Nazwa Zawartość / Mechanika 0 Kontekst Tytuł "Wesprzyj \[Nazwa\]", Awatar twórcy 32x32px, przycisk X 1 Kwota Przyciski szybkie ($1, $5, $10, $20, $50) \+ Pole własne (walidacja min/max). Live conversion na USDC/ETH (oracle co 30s). font-feature-settings: "tnum" 2 Metoda Akordeon z 3 opcjami: Karta (Circle), Web3 Wallet, Saldo TipJar 3a Formularz Karty Circle Elements (PCI DSS). Stylowanie przez style object. Checkbox "Zapisz kartę". 3b Formularz Web3 Przycisk "Połącz portfel" (RainbowKit/Web3Modal). Stan połączony: skrócony adres 0x12...89AB lub ENS (fan.eth), saldo USDC. 3c Saldo TipJar Wyświetlenie dostępnego salda. Przycisk "Zapłać z salda". 4 Opcje Dodatkowe Akordeon: Wiadomość (textarea, limit 200 znaków, DOMPurify), Checkbox "Proof of Support NFT" (domyślnie zaznaczony), Checkbox "Anonimowo" 5 Podsumowanie Tabela: Kwota \+ Opłata platformy (0%) \+ Gas Fee (szacowane). Przycisk finalny \--gold-400. Klauzula MiCA (microcopy). 1.2 Wybór Sieci (Network Warning) Właściwość Wartość Obsługiwane sieci Polygon (chainId: 0x89), Ethereum Mainnet (0x1) – z priorytetem L2 Wykrywanie wagmi useChainId Nieprawidłowa sieć Żółty pasek (--warning-base): "Zmień sieć na Polygon, aby kontynuować." Akcja Przycisk "Zmień sieć" → window.ethereum.request({ method: 'wallet\_switchEthereumChain', params: \[{ chainId: '0x89' }\] })**

2. Stany Transakcji Web3 (Transaction Lifecycle)  
   Zgodnie z tech\_modal.pdf (str. 7-8).  
...
3. ---
   ...
   Abstrakcja Konta (ERC-4337) i Paymaster  
   Zgodnie z tech\_modal.pdf (str. 9\) i tech\_fan\_profil.pdf (str. 7).  
...
4. ---
   ...
   Wyświetlanie Adresów i ENS  
   Zgodnie z Master Plan UI (str. 9\) i tech\_katalog\_3part.pdf (str. 20).  
   Właściwość Wartość  
...
5. ---
   ...
   Zgodność z MiCA (Markets in Crypto-Assets)  
   Zgodnie z tech\_modal.pdf (str. 2\) i tech\_knowledge.pdf (str. 3-4).Wymóg MiCA Implementacja w UI  
...
6. ---
   ...
   Checklista Implementacyjna Modala Płatności  
...
3. ---
   ...
   Interlinia 1.7  
   Kolor tekstu \--text-secondary (var(--teal-50), nie ostra biel)  
   ...
   Właściwość Wartość  
   Wyzwalacz Terminy techniczne w z przerywanym podkreśleniem \--gold-400  
...
6. ---
   ...
   Checklista Komponentów Centrum WiedzyAtom / Molekuła Kluczowe Właściwości  
   ...
   GlossaryTooltip z data-tooltip, dynamiczne pozycjonowanie  
...
| \--border-focus | var(--purple-300) | **Pierścień nawigacji klawiaturowej** |
| \--action-primary-bg | var(--gold-400) | Tło głównych przycisków CTA |
| \--action-primary-text | var(--teal-900) | **Kolor tekstu na złotym tle** ( |
| Krytyczne dla |  |  |
| WCAG) |  |  |

## **| \--action-secondary-bg | var(--purple-300) | Tło przycisków drugorzędnych | ⚠️**
...
6. ---
   ...
   Breakpointy Responsywne (Mobile-First)  
   Nazwa Min. Szerokość Zastosowanie  
   xs 320px Mikro urządzenia  
   sm 640px Punkt krytyczny: Modale \-\> Bottom Sheets  
   md 768px Tablety  
...
Tekst musi być \`--teal-900\` (kontrast \`11.2:1\` \= AAA).  
\#\#\#\# 1.3 Wariant: Secondary (Fioletowy / Outline)
...
| \*\*Default\*\* | \`transparent\` lub \`var(--error-dark)\` | \`var(--error-base)\` |  
| \*\*Hover\*\* | \`rgba(255, 82, 82, 0.1)\` | \`var(--error-base)\` |  
| \*\*Focus\*\* | \`transparent\` | Pierścień \`var(--error-base)\` |\*\*Uwaga:\*\* Nie używaj czystej czerwieni \`\#FF0000\`. W trybie ciemnym powoduje wibrację  
optyczną.  
\#\#\#\# 1.5 Wariant: Floating Action Button (FAB)
...
| \*\*Error\*\* | \`--error-base\` | \`0 0 0 4px rgba(255, 82, 82, 0.25)\` | Kolor \`--error-base\` |  
| \*\*Success\*\* | \`--success-base\` | \`none\` | Kolor \`--success-base\` |  
| \*\*Disabled\*\* | \`--teal-700\` (przerywane) | \`none\` | Opacity \`0.4\` |\*\*Walidacja:\*\* Tekst błędu pod polem w kolorze \`--error-light\` (\`\#FFB4AB\`). Dodatkowo  
...
Odznaka umieszczona w prawym dolnym rogu (\`bottom: 7.3%\`, \`right: 7.3%\`). Aby uniknąć  
brzydkiego obramowania, stosujemy \*\*CSS Masking\*\*:css  
/\* Wycięcie w awatarze pod odznakę \*/  
\-webkit-mask-image: radial-gradient(  
circle at 85% 85%,  
transparent calc(var(--badge-size) / 2 \+ 2px),  
...
HelpfulnessButtons Tak/Nie, textarea przy "Nie"
EmptyState Ilustracja + komunikat + sugerowane linki
Skeleton Article Shimmer na blokach tekstu
---
