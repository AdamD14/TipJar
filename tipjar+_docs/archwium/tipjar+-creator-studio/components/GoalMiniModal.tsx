...
export default function GoalMiniModal({ title, current, target, isOpen, onClose }: GoalMiniModalProps) {
  ...
  return (
    <motion.div 
      ...
      className="fixed bottom-24 right-6 w-72 bg-[#003737]/90 backdrop-blur-2xl border border-white/10 rounded-[2.5rem] p-6 shadow-2xl z-[90] ring-1 ring-white/20 overflow-hidden"
    >
      <div className="relative z-10 space-y-4">
        <div className="flex items-center justify-between">
           <div className="flex items-center gap-2">
              <div className="w-6 h-6 rounded-lg bg-[#FFD700] flex items-center justify-center text-[#003737]"><Zap size={14} fill="currentColor" /></div>
        ...
        <div className="h-1.5 bg-white/5 rounded-full overflow-hidden p-0.5">
           <div 
             className="h-full bg-[#FFD700] rounded-full transition-all duration-[1500ms]" 
             style={{ width: `${percent}%` }} 
           />
        </div>
