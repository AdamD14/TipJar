Krok 1: Fundamenty Systemu (Globalne Tokeny)
...
---
## 🧬 Krok 7: Specyfikacja Abstrakcyjnych Grafik Tła 3D

### 1. Filozofia "Soft Tech" i Cel Strategiczny

Grafiki 3D w TipJar+ nie pełnią funkcji dekoracyjnej. Są **narzędziem inżynierii zaufania**, którego celem jest redukcja lęku finansowego (financial anxiety) i budowanie psychologicznego przywiązania do platformy. Odrzucamy agresywną estetykę cyberpunkową i surowe, techniczne formy na rzecz **miękkich, organicznych brył**, które w sposób niewerbalny komunikują bezpieczeństwo, płynność i globalną sieć. Grafika musi **tłumaczyć abstrakcyjny świat kryptografii na zrozumiały, przyjazny język wizualny**.

### 2. Psychologia Kształtów – Co Jest Dozwolone

| Rodzaj Kształtu | Zastosowanie | Uzasadnienie Psychologiczne |
| :--- | :--- | :--- |
| **Obłe sfery, kule** | Główny element kompozycji (Motyw "Połączenie") | Jedność, bezpieczeństwo, harmonia. Brak początku i końca sugeruje nieskończoność. |
| **Organiczne, miękkie fale** | Tła sekcji, pasy rozdzielające (Motyw "Globalność") | Spokój, naturalność, brak tarcia. Redukuje napięcie. |
| **Wygładzone kryształy** (ze ściętymi, zaoblonymi krawędziami) | Akcenty wzrostu, widgety KPI (Motyw "Wzrost") | Wzrost wartości, precyzja, długoterminowa transformacja. Krystalizacja zysków. |
| **Ostre wielokąty, kolce** | **Kategorycznie zabronione** (poza krytycznymi alertami) | Ryzyko, agresja, zagrożenie. Wywołują podświadomy stres. |

### 3. System Kolorystyczny dla Grafik 3D

| Rola | Kolor/Tony | Token CSS (Odniesienie) | Zastosowanie w Renderze 3D |
| :--- | :--- | :--- | :--- |
| **Płótno / Tło** | Głęboki turkus, czerń morska | `--teal-800` (`#003737`), `--teal-900` (`#001F1F`) | Absolutna baza, tło dla brył. Nigdy czysta czerń (`#000000`). |
| **Światło / Akcent** | Złoto (punktowo!) | `--gold-400` (`#FFD700`) | Odbicia, kaustyki, cienkie linie. Ma być **punktowe** – unikać przesycenia (inflacji wizualnej). |
| **Technologia / Głębia** | Fiolet, ciemny fiolet | `--purple-300` (`#9D4EDD`), `#4D194D` | Gradienty na sferach, akcenty technologiczne. Ciemny fiolet jako drugi biegun gradientu. |
| **Wzrost / Sukces** | Szmaragd (rozjaśniony i żółty) | `--success-base` (`#00E676`) | Mikro-akcenty na kryształach "Wzrostu". Musi być jaśniejszy od standardowej zieleni. |

### 4. Trzy Motywy Tematyczne

#### 4.1 Motyw: Połączenie (Connection)

| Atrybut | Specyfikacja |
| :--- | :--- |
| **Metafora** | Fuzja, jedność, przepływ wartości P2P. |
| **Forma 3D** | Organiczne, obłe kule łączące się i przenikające (efekt "metaballs"). Miękkie, bez ostrych krawędzi odcięcia. |
| **Materiał** | Matowa powierzchnia z głębokim rozpraszaniem światła pod powierzchnią (subsurface scattering). Bez ostrych, lustrzanych odbić (specular highlights). |
| **Kolorystyka** | Głęboki turkus (`#003737`) i ciemny fiolet (`#4D194D`). Przejścia między barwami płynne, gradientowe. |
| **Kompozycja** | Asymetryczna, lewitująca w pustce. Krawędzie zewnętrzne wtapiają się w tło strony (alpha fading). Strefy styku kul rozmyte. |
| **Zastosowanie** | **Sekcje Hero**, ekrany powitalne, tła onboardingowe. |

#### 4.2 Motyw: Wzrost (Growth)

| Atrybut | Specyfikacja |
| :--- | :--- |
| **Metafora** | Krystalizacja, akumulacja wartości, długoterminowy rozwój. |
| **Forma 3D** | Struktury krystaliczne / geody. Krawędzie **muszą być ścięte i zaoblone** (bevel/fillet) – nigdy ostre i szpiczaste. |
| **Materiał** | Zróżnicowana przezroczystość – od pełnego szkła po zmętnienie. Wewnętrzne inkluzje sugerujące głębię i transparentność. |
| **Kolorystyka** | Dominanta: złoto (`#FFD700`). Akcenty: szmaragd (`#00E676`). Ciepłe, miękkie światło rozproszone (area lights), brak twardych cieni. |
| **Zastosowanie** | **Dashboardy**, karty KPI, widgety analityczne, tła dla sekcji finansowych. |

#### 4.3 Motyw: Globalność (Global)

| Atrybut | Specyfikacja |
| :--- | :--- |
| **Metafora** | Decentralizacja, nieskończona sieć, globalny zasięg bez granic. |
| **Forma 3D** | Wielopoziomowe, półprzezroczyste mapy topograficzne lub łagodne fale. Izolinie, płynne wzniesienia. |
| **Materiał** | Przestrzenne warstwy z trybami mieszania (Screen, Add, Overlay) i niskim kryciem (15-30%). Rozmycie wgłębne (Depth of Field) na krawędziach. |
| **Kolorystyka** | Wyłącznie gama turkusowa: `teal-900` → `teal-500` → `teal-200` → `teal-50`. |
| **Kompozycja** | Brak zamkniętych granic i sztywnych punktów. Forma rozlewa się poza kadr. **Nigdy nie używać kuli ziemskiej, kontynentów ani laserów.** |
| **Zastosowanie** | **Katalog Twórców**, ekrany "Odkrywaj", Centrum Wiedzy. |

### 5. Integracja z Interfejsem (Glassmorphism i Z-Index)

Grafika 3D jest zawsze **tłem** i podlega ścisłej hierarchii warstw.

| Warstwa | Z-Index | Funkcja |
| :--- | :--- | :--- |
| **Płótno 3D** | `--z-base` (0) | Wyeksportowany plik graficzny / wideo / WebGL. Osadzony na sztywno za wszystkim. |
| **Szklana Powłoka UI** | `--z-elevated` (10) | Karty, panele, kontenery z treścią. Nakładane na grafikę 3D z wykorzystaniem tokenów Glassmorphism: `--glass-overlay`, `--glass-blur`, `--glass-border`. |
| **Backdrop Modala** | `--z-backdrop` (500) | Przy otwarciu modala grafika 3D jest przyciemniana i rozmywana (`rgba(0,31,31,0.85)` + `blur(4px)`). |
| **Treść Pierwszoplanowa** | Dynamicznie wyżej | Wszelki tekst musi być na szklanej powłoce. **Nigdy bezpośrednio na renderze 3D.** |

### 6. Wymagania Techniczne dla Plików Graficznych

#### 6.1 Zapobieganie "Bandingowi" na Ciemnych Tłach

Na głębokich, ciemnych gradientach (przejścia od `teal-800` do `teal-900`) pojawia się efekt "paskowania" (color banding) spowodowany kompresją i niską bitowością ekranów.

- **Działanie:** Do każdego wyeksportowanego renderu 3D należy dodać **subtelny, równomierny szum (noise/grain)**.
- **Cel:** Szum imituje fakturę kliszy fotograficznej, rozprasza piksele (dithering) i całkowicie eliminuje widoczność pasów przejściowych. Dodaje również wrażenia "analogowej" głębi i materialności.

#### 6.2 Zapewnienie Dostępności (WCAG) – Zasada "Safe Zones"

Wygenerowany obraz 3D nie może zakłócać czytelności treści.

- **Kompozycja:** Główna masa, detale i jaskrawe refleksy świetlne w renderze 3D muszą być **wypchnięte z centrum kadru** w kierunku peryferiów.
- **Cel:** Pozostawienie w centrum ekranu wizualnie wyciszonej, jednolitej tonalnie "Bezpiecznej Strefy" (Safe Zone). To w niej, na szklanej powłoce, będzie osadzany tekst i komponenty UI.
- **Kolor Safe Zone:** Bazowy, głęboki turkus `--teal-800` bez gwałtownych skoków luminancji.

#### 6.3 Wibracja Optyczna (Chromatic Vibration)

- **Zagrożenie:** Bezpośrednie sąsiedztwo wysoce nasyconych, jaskrawych kolorów (np. `#00E676`) z głębokim, ciemnym tłem powoduje męczący efekt wibracji.
- **Zapobieganie:** Ostre, nasycone plamy w renderze 3D nie mogą mieć twardych krawędzi. Przejście od jaskrawego punktu (np. odbicia) do ciemnego tła musi odbywać się przez **szeroki, miękki gradient dyfuzyjny**.

#### 6.4 Typografia a Tło – Nigdy Bez Pośrednika

- **Zakaz:** Niedopuszczalne jest wyświetlanie czystej, białej (`#FFFFFF`) typografii bezpośrednio na zmiennym, szczegółowym renderze 3D.
- **Wymóg:** Wszystek tekst akapitowy używa koloru `--text-secondary` (`#D6EBEB` – złamana biel turkusowa) i znajduje się na **szklanej powłoce z aktywnym rozmyciem**, która odcina go od graficznego tła.

### 7. Strategia Rozmieszczenia w Widokach Aplikacji

| Widok / Sekcja | Rekomendowany Motyw | Funkcja Psychologiczna |
| :--- | :--- | :--- |
| **Landing Page / Hero** | Globalność + Połączenie (fuzja) | Pierwsze wrażenie luksusu, bezpieczeństwa i globalnego zasięgu. |
| **Dashboardy, KPI** | Wzrost | Wizualizacja akumulacji, stabilnego rozwoju i namacalności zysków. |
| **Puste Stany (Empty States)** | Połączenie (zredukowane, wyciszone) | Łagodzenie frustracji z powodu braku danych, budowanie oczekiwania. |
| **Katalog / Odkrywaj** | Globalność | Sugestia nieskończonej skali i różnorodności ekosystemu. |

### 8. Checklista dla Artysty 3D

- [ ] Czy wszystkie krawędzie brył są obłe lub ścięte (bevel)? (Ostre kąty niedozwolone).
- [ ] Czy złoto użyte jest tylko jako punktowy akcent, a nie dominująca powłoka?
- [ ] Czy główna masa kompozycji wypchnięta jest na peryferia, zostawiając centrum jako Safe Zone?
- [ ] Czy do eksportu dodano subtelny szum (noise) w celu eliminacji bandingu?
- [ ] Czy eksportowany plik ma format wspierający przezroczystość (np. PNG/WEBP z kanałem alfa) dla płynnego wtapiania się w tło?
- [ ] Czy grafika jest testowana pod kątem czytelności tekstu na nałożonych, szklanych panelach?
