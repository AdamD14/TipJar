...
--action-primary-text var(--teal-800) Kolor tekstu na złotym tle (⚠ Krytyczne dla WCAG)
...
⚠ Obszar dotykowy: Dla Small minimalny obszar kliknięcia to 44px (realizowane przez
pseudoelement).
...
Twórcy (Creator) Awatar 64px, nazwa, handle, statystyki Złoty checkmark (Verified), przycisk
"Obserwuj"
...
:::info Tło rgba(102, 217, 232, 0.1), obramowanie --info-base, ikona ℹ
...
| `--border-focus` | `var(--purple-300)` | **Pierścień nawigacji klawiaturowej** |
| `--action-primary-bg` | `var(--gold-400)` | Tło głównych przycisków CTA |
| `--action-primary-text` | `var(--teal-800)` | **Kolor tekstu na złotym tle** (⚠ Krytyczne dla
WCAG) |
| `--action-secondary-bg` | `var(--purple-300)` | Tło przycisków drugorzędnych |
...
**Uwaga:** Nie używaj czystej czerwieni `#FF0000`. W trybie ciemnym powoduje wibrację
optyczną.
...
**Walidacja:** Tekst błędu pod polem w kolorze `--error-base` (`#FFB4AB`). Dodatkowo
