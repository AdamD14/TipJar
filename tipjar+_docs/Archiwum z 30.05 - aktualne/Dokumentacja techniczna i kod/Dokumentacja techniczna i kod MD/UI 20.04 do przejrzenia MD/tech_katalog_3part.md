...
(tooltips) ułatwszając eksplorację bez konieczności opuszczania widoku głównego.
...
Część III: Architektura Katalogu
Twórców (Explore / Odkrywaj)

1. Cel Strategiczny i Kontekst Biznesowy

Katalog Twórców funkcjonuje jako mechanizm odkrywania (discovery engine) i główny strumień
kierujący ruch od niezaangażowanych użytkowników do profili poszczególnych twórców, gdzie
następuje proces finansowej konwersji. Podstawowym celem biznesowym tej przestrzeni jest
błyskawiczne odnalezienie twórcy zgodnego z preferencjami darczyńcy.
Sukces tego modułu uzależniony jest od trzech nadrzędnych metryk: czasu wyszukania
(redukcja dystansu do profilu poniżej 3 kliknięć), współczynnika konwersji ("Click-Through Rate"
z karty na profil bazowy) oraz niezachwianej optymalizacji pod kątem wyszukiwarek (SEO).
Katalog nie tylko indeksuje tysiące twórców Web3 dla robotów Google, ale stanowi
jednocześnie pierwszy kontakt wzrokowy fana z ekosystemem, w którym jakikolwiek
nieporządek lub opóźnienia powodują drastyczny wzrost współczynnika odrzuceń (Bounce
Rate).

2. Architektura Informacji i Układ (Layout)

Aby skutecznie zarządzać listami wielkości setek czy tysięcy pozycji, należy wystrzegać się
układów typu Masonry, które sprawdzają się na Ścianach Fanów, ale zaburzają przewidywalne
skanowanie bazodanowe wzrokiem. Do listowania wykorzystany zostanie klasyczny,
symetryczny Grid.

Paradygmat Desktopowy (≥1024px)

Architektura desktopowa implementuje pełnoekranowe podejście narzędziowe:

1.  Nagłówek i Wyszukiwarka: Centralnie zlokalizowany główny znacznik H1 z wezwaniem
do odkrywania, podparty masywnym polem wyszukiwania zajmującym szerokość rdzenia.

2.  Pasek Szybkich Filtrów: System horyzontalnie ułożonych "chipów" kategorii (np.

Muzyka, Gaming).

3.  Zaawansowane Filtrowanie: Otwierany opcjonalnie z góry panel rozwijany, pozwalający

na skrzyżowanie filtrów sortowania (np. najpopularniejsi), wyboru języka oraz ukrywania
niezweryfikowanych portfeli.

4.  Siatka Kart (Grid): Regularny rozkład 4-kolumnowy, w którym każda komórka ma taką

samą wysokość. Pozwala to na szybkie trawienie zawartości.

5.  Nawigacja Stronicowania: Klasyczna Paginacja oparta o numery stron z pominięciem
"nieskończonego skrolowania", by zapewnić idealne indeksowanie zawartości przez
roboty indeksujące.

Transformacja Mobilna (<640px)

W środowiskach rzutni mobilnych zachodzi linearyzacja zapobiegająca zatłoczeniu ekranu.
Złożony pasek zaawansowanych filtrów ukrywany jest pod przyciskiem "Filtry", który na żądanie
aktywuje dolną szufladę krawędziową (Bottom Sheet). Szybkie filtry przybierają postać poziomej
strefy przewijanej palcem z uciętą krawędzią (overflow-x auto), zachęcającą do swipe'owania.
Siatka (Grid) ulega transformacji do 2 zoptymalizowanych kolumn, a moduł stronicowania
zostaje zredukowany do oszczędnych przycisków typu "Poprzednia/Następna" wspartych
czytelnym wskaźnikiem pozycji.

3. Inżynieria Wyszukiwania i Optymalizacja Filtracji
(Query Params)

Implementacja natychmiastowego wyszukiwania na listach (Live Search) wymaga precyzyjnego
zarządzania obciążeniem i zminimalizowania zbędnych cykli aktualizacji. Atom SearchInput
połączono z asynchronicznym opóźnieniem sygnału (debounce) ustawionym na przedział rzędu
300ms. Oznacza to, że silnik filtrujący nie przeciąża przeglądarki ani serwera podczas
szybkiego wklepywania liter; zapytanie odpalane jest ułamek sekundy po zatrzymaniu pisania.
Zarządzanie stanem (State Management) dla filtrów krzyżowych opiera się niemal w całości na
API parametrów adresów URL. Rozwijany układ operuje na haku Next.js 15 useSearchParams.
Zaznaczenie odpowiedniej kategorii (np. muzyki) oraz popularności wywołuje aktualizację paska
adresu za pośrednictwem funkcji router.push(url, { scroll: false }) do postaci typu
?category=music&sort=popular. Jest to element absolutnie kluczowy, gdyż umożliwia:

●  Błyskawiczne współdzielenie linku pomiędzy użytkownikami z już nałożonymi restrykcjami

wyszukiwania.

●  Zapobieganie utraty wybranej filtracji po przejściu z powrotem z widoku konkretnego

twórcy do katalogu.

●  Wspomaganie głębokiego linkowania (Deep Linking) dla kampanii marketingowych.

4. Architektura Siatki (Grid CSS) i Karta Twórcy

W przeciwieństwie do Profilu Publicznego, gdzie zróżnicowana zawartość opinii narzucała
renderowanie bezstanowe przy użyciu biblioteki TanStack Virtual w rzędach kaskadowych ,
siatka na stronie Odkrywaj korzysta z natywnych rozwiązań CSS zapewniających stabilność
bloków.
Rdzeń siatki zdefiniowano poniższym schematem właściwości:
...
Zapewnia to bezpieczne "zawijanie się" (wrapping) komórek do formatów dwu lub
jednokolumnowych, chroniąc kartę twórcy przed niebezpiecznym obcięciem na mniejszych
urządzeniach, bez konieczności definiowania nadmiernych Media Queries.

Karta Twórcy jako Złożona Molekuła

Karta twórcy stanowi zintegrowaną strukturę o stałej (niezmieniającej się asynchronicznie)
wysokości około 320px. Obiekt wyśrodkowuje na tarczy awatar (96x96px), wsparty nagłówkiem
H4 z dynamicznie przycinanym tekstem (maksymalnie dwie linie), minimalizując wizualny
bałagan. Aby podtrzymać koncepcję wizualną projektów premium, umieszczony wewnątrz
element interaktywny otrzymuje płynny efekt unoszenia (translateY(-4px)) za pośrednictwem
parametru najechania myszą wariantu Hover, ze wsparciem narastającego cienia rzutowanego
z poziomu --shadow-1 na --shadow-2. Przycisk "Zobacz profil", z domyślnego wariantu
przezroczystego konturu, płynnie zalewa się w kolorze ostrzegawczym wezwania do działania
--gold-400.
Dla zredukowania obciążenia operacyjnego po stronie klienta, z uwagi na stronicowanie po
20-30 kart w siatce, zaniechano skomplikowanej infrastruktury wirtualizacyjnej z bibliotek
zewnętrznych na rzecz natywnego ładowania. W razie dłuższego braku sieci, ładowanie
zaspokajane jest komponentem SkeletonCard wyposażonym w migoczący stan typu Shimmer,
co bezwarunkowo chroni przed załamaniami układu strony (CLS).

5. Integracje Web3: Zabezpieczenia, Tożsamość ENS i
Społeczny Dowód Słuszności

Katalog Twórców operuje jako zaawansowane środowisko weryfikacyjne dla przestrzeni, w
których portfele bywają często ukrywane pod fałszywymi pseudonimami (spoofing). Głównym
elementem budowania zaufania na zewnątrz i redukowania paraliżu decyzyjnego darczyńcy są
identyfikatory tożsamości.
Molekuła "VerificationBadge" implementuje złotą ikonę dla każdego twórcy połączonego
protokołami Web3. Aby proces nie eksponował losowych i zniechęcających ciągów znaków
Ethereum, implementacja frontendowa ponownie opiera się na wywoływaniu funkcji
środowiskowych z pakietu viem — w szczególności asynchronicznej metody getEnsName
nakładanej na adres domyślnego twórcy. W rygorze zabezpieczeń uwzględniono, iż proces
odpytywania uniwersalnego kontraktu wspierany jest wewnątrz algorytmem normalize,
pozbawiającym złośliwe ataki homograficzne swojej siły.
Zabieg "Social Proof" (Społecznego dowodu) manifestuje się obok awatarów w postaci
liczników metryk. Widoczna, sumaryczna kwota przychodów danego konta w walucie USDC
zniechęca oszustów, równocześnie nadając karcie wiarygodność ekonomiczną wymuszającą na
użytkowniku szybszą ścieżkę kliknięcia (Click-Through).

6. Architektura Next.js 15: Renderowanie Hybrydowe
w Katalogach

Rozwiązanie problematyki tysięcy twórców listowanych na rzutniach wymaga zintegrowania
odpowiednich dróg renderowania po stronie Next.js App Routera. Platforma wdraża w tym
przypadku kompromis idealny pomiędzy gotowymi strukturami a elastycznym zapytaniem.
Lista Twórców jest dostarczana robotom optymalizacyjnym (Google, DuckDuckGo) w całości
przygotowanym do odczytu pakiecie statycznym za pośrednictwem SSG (Static Site
Generation) przy użyciu standardu generateStaticParams. Rozszerza ją dyrektywa czyszczenia
i odnawiania infrastruktury Incremental Static Regeneration (ISR) z parametrem buforowym
revalidate: 3600 (odświeżenie co godzinę). Taka kombinacja sprawia, że plik HTML dla całego
katalogu dostarczany jest prosto z globalnych struktur krańcowych CDN (bez kosztownego
odpytywania bazy danych na każdy wgląd), zapewniając indeksowanie na poziomie milisekund.
Operacje filtrowania oraz sortowania wykluczono z architektury serwerowej na rzecz płynności
interfejsowej Client-Side Rendering (CSR). Przesłane w buforze initialData dane są miksowane
w czasie rzeczywistym z odczytami parametru URL. Zastosowanie takich wzorców odcina nas
od skomplikowanych przeładowań strony (Server Components) na każdy najmniejszy wybór
języka w panelu, pozwalając utrzymać niespotykaną lekkość działania bez obniżenia pozycji
wyników organicznych.

7. Wymagania Dostępności (WCAG 2.2)

Sekcja Odkrywaj wdraża nadrzędne wymogi narzucone przez dyrektywy standardu WCAG.
Złote zasady objęły przede wszystkim kontrasty oraz zarządzanie celami interakcji dla
powtarzalnych elementów, jakimi są setki kart twórców na urządzeniach dotykowych.

1.  Zgodnie z wymaganiami sukcesu (Focus Appearance) dla skomplikowanych układów
interakcyjnych, kategorycznie odrzucono wyłączanie obramowań. Elementy kluczowe
wyposażone zostały we wskaźnik focus-visible, co oznacza pojawienie się w obszarze
interfejsu klawiaturowego grubego na 2 piksele obrysu (--purple-300) ze zmodyfikowanym
przesunięciem dystansowym (offset), wyodrębniającym dany element bez maskowania.

2.  Minimalne pole rażenia celownika dla klikalnych chipów, paginacji oraz sekcji filtra

rozciągnięto niewidocznymi warstwami maskującymi (np. w pseudo-elementach ::after)
powyżej rozmiaru okna ewaluacyjnego równego 44x44 piksele, absolutnie niwelując błędy
na ciasnych wyświetlaczach telefonu.

3.  Animowane wznoszenie kart z rzucaniem ciężkiego cienia zostaje zneutralizowane
powszechną regułą @media (prefers-reduced-motion: reduce), wspierając osoby
dotknięte dolegliwościami błędnikowymi.

8. Checklista Implementacyjna (Backlog dla
Inżynierów)

Poniżej zagregowano końcowy podział odpowiedzialności modułowej jako przygotowanie do
implementacji warstwy front-endowej.
Atomy Krawędziowe (Elementy Niskiego Rzędu)

●  Podstawowy znacznik H1 w zadanym foncie systemowym.

●  Asynchroniczny obszar SearchInput połączony z funkcją debounce.
●  Mikroelementy ChipFilter ze wsparciem zmiennej kolorystyki oraz moduł sortowania

SortDropdown.

●  Obszar stronicowania (Pagination) i awaryjne okna braku zawartości (EmptyState z

wezwaniem do wyczyszczenia konfiguracji).

Molekuły i Obiekty Widokowe

●  Struktura AdvancedFiltersPanel przełączana z modelu Dropdown (dla desktopu) do

Drawer (na telefonach).

●  CreatorCard – centralny organizm ze zintegrowanym poborem wskaźników sieci ENS i

licznikiem portfela.

●  Pełna klasa kontenerowa CreatorsGrid realizująca algorytm zapisu kaskadowego w CSS

na powielonych blokach auto-fill.

●  Agregat wizualny i router ścieżki (Strona główna katalogu), zarządzający buforami i

przesyłem zmiennych adresu URL w formacie ?category=x&sort=y.

Cytowane prace

1. react-virtualized vs. react-window - LogRocket Blog,
https://blog.logrocket.com/react-virtualized-vs-react-window/ 2. getEnsAddress - Wagmi,
https://wagmi.sh/core/api/actions/getEnsAddress 3. GitHub - ensdomains/react-ens-address:
React Component to resolve ENS names or reverse resolve addresses,
https://github.com/ensdomains/react-ens-address 4. Understanding Success Criterion 2.4.7:
Focus Visible | WAI - W3C, https://www.w3.org/WAI/WCAG22/Understanding/focus-visible.html
5. A guide to designing accessible, WCAG-conformant focus indicators - Sara Soueidan,
